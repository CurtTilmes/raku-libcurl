...
multi check($code) is export is hidden-from-backtrace
{
    die X::LibCurl.new(:$code)
}
...
my %opts =
    ...
    default-protocol     => (CURLOPT_DEFAULT_PROTOCOL,     CURLOPT_STR      ),
    ...
    path-as-is           => (CURLOPT_PATH_AS_IS,           CURLOPT_BOOL     ),
    ...
    request-target       => (CURLOPT_REQUEST_TARGET,       CURLOPT_STR      ),
    ...
    unix-socket-path     => (CURLOPT_UNIX_SOCKET_PATH,     CURLOPT_STR      ),
...
class LibCurl::Easy
{
    ...
    method formadd(Str :$name, :$contents, Str :$filename,
                   Str :$contenttype, Str :$filecontent, Str :$file)
    {
        $!mime //= $!handle.mime-init;

        my $part = $!mime.addpart;
        check $part.name($name) if $name;
        check $part.data($contents) if $contents;
        check $part.type($contenttype) if $contenttype;
        check $part.filedata($filecontent) if $filecontent;
        check $part.filedata($file) if $file;
        check $part.filename($filename // $file) if $filename // $file;
